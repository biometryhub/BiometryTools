...
verify_sunset_sunrise_params <- function(datetime, latitude, longitude) {
  verify_date(datetime)
  verify_between(latitude, lower = -90, upper = 90)
  verify_between(longitude, lower = -180, upper = 180)
}
...
verify_date <- function(..., var_names = NULL) {
  verify_data_type(lubridate::is.timepoint, "a datetime object", var_names, ...)
}
