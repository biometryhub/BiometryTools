...
#' @param output One of `online` (the default), `gist` or `local`. Saves a list of
#'   installed packages to the chosen location, and provides instructions on how to
#'   use this to (re)install the packages elsewhere. See details for more information.
#' @param expiry Expiry for online file store in days. Weeks can be given with `w`,
#'   or months with `m`. Default is 7 days. Will be ignored if `output` is not `online`.
#' @param filename Filename for the local output file. Ignored if `output` is not
#'   set to `local`.
#' @param list_remotes Logical (default `TRUE`). Check for any packages installed
#'   from repositories other than CRAN, and output instructions to reinstall.
...
#'
#' @details If `output` is `online`, the resulting list of currently installed packages
#' is stored on [https://file.io](https://file.io) for the time specified in `expiry`,
#' or until the URL is first accessed. Note that either visiting the URL in a browser
#' or sourcing the URL via R count as access, and it will be removed after either.
#'
#' If `output` is `local`, an R script file (`.R`) is saved to the current working
#' directory, which can be transferred manually to another machine.
#'
#' If `output` is `gist`, the script is uploaded as a GitHub gist (requires the
#' `gistr` package).
#'
#' @return Invisibly returns the command to run on the target machine. If `quiet = FALSE`,
#'   also prints instructions to console.
#' @export
#'
...
{to_install}")
    ...
    if (output == "online") {
        r <- httr::POST(glue::glue("https://file.io/?expires={expiry}"), body = list(file = to_install))
